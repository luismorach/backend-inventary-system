...
class CoinsController {
    ...
    async getCoinByCountry(req, res) {
        ...
        const response = await database_1.default.query('select distinct on (c.currency_code) c.country,c.country_code,l.language,' +
            'l.language_code,c.currency_code,a.currency from countries c  join languages l  on ' +
            'c.language_code= l.language_code join all_coins a on ' +
    ...
    async getCoins(req, res) {
        const response = await database_1.default.query('select * from currencies order by currency asc');
        res.json(response.rows);
    }
    async getSecondariesCoins(req, res) {
        const response = await database_1.default.query('select distinct on (a.currency_code) ' +
