...
class CajasController {
    async getCajas(req, res) {
        console.log('pasando');
        const response = await database_1.default.query('SELECT * FROM registers WHERE id_register!=0 ORDER BY name_register ASC');
    ...
    async getCajaById(req, res) {
        ...
        const response = await database_1.default.query('SELECT * FROM registers WHERE id_register=$1 ORDER BY name_register ASC', [id_register]);
    ...
    async createCaja(req, res) {
        ...
        await database_1.default.query('INSERT INTO registers (name_register,state_register) values($1,$2)', [name_register, state_register]);
        res.status(200).json({
    ...
    async actualizarCaja(req, res) {
        const id_register = parseInt(req.params.id_register);
        const { name_register, state_register } = req.body;
        await database_1.default.query('UPDATE registers SET name_register=$1,state_register=$2 WHERE id_register=$3', [name_register, state_register, id_register]);
        res.status(200).json({
