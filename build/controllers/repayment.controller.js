...
class DevolucionesController {
    async getDevoluciones(req, res) {
        const response = await conection_1.default.query('SELECT * FROM repayment ORDER BY date,time DESC ');
        res.json(response.rows);
    }
    async getDevolucionById(req, res) {
        const id_repayment = parseInt(req.params.id_repayment);
        const response = await conection_1.default.query('SELECT * FROM repayment JOIN users ON ' +
    ...
    async getDevolucionesById(req, res) {
        ...
        const response = await conection_1.default.query('SELECT * FROM repayment r  JOIN users u ON ' +
    ...
    async getDevolucionBuy(req, res) {
        ...
        const response = await conection_1.default.query('SELECT * FROM repayment ' +
    ...
    async getProductosDevolucionById(req, res) {
        const id_repayment = parseInt(req.params.id_repayment);
        const response = await conection_1.default.query('SELECT * FROM buy_products WHERE id_repayment=$1', [id_repayment]);
        if (response.rowCount > 0) {
            res.json(response.rows);
        }
    }
    async createDevolucionCompra(req, res) {
        ...
        let response = await conection_1.default.query('INSERT INTO repayment (type,id_buy,quantity,' +
    ...
    async createDevolucionVenta(req, res) {
        ...
        let response = await conection_1.default.query('INSERT INTO repayment (type,id_sell,quantity,' +
            'buy_price,total,currency_code,exist_quantity,weighted_averages_sell,id_user,id_product,' +
    ...
    async actualizarDevolucion(req, res) {
        ...
        await conection_1.default.query('UPDATE repayment SET otal_buy=$1,id_provider=$2,id_user=$3 WHERE id_repayment=$4', [type, id_operation, quantity, price, total, currency_code, exist_quantity,
    ...
    async eliminarDevolucion(req, res) {
        const id_repayment = parseInt(req.params.id_repayment);
        await conection_1.default.query('DELETE FROM repayment WHERE id_repayment=$1', [id_repayment]);
        res.status(200).json({
