...
class VentasController {
    async getVentas(req, res) {
        const response = await conection_1.default.query('SELECT * FROM sells ORDER BY time DESC ');
    ...
    async getVentaById(req, res) {
        ...
        const response = await conection_1.default.query('SELECT distinct on (sells.id_sell)* FROM sells JOIN users ON ' +
    ...
    async getVentasByDate(req, res) {
        ...
        const response = await conection_1.default.query('SELECT distinct on (sells.id_sell)* FROM sells ' +
            'JOIN users ON sells.id_user=users.id_user JOIN clients ON sells.id_client=clients.id_client ' +
            'join currencies c on c.currency_code=sells.currency_code ' +
            'AND date>=$1 and date<=$2 ORDER BY sells.id_sell DESC', [initialDate, endDate]);
        res.json(response.rows);
    }
    async getVentaByDate(req, res) {
        const initialDate = req.params.initialDate;
        const endDate = req.params.endDate;
        const response = await conection_1.default.query('SELECT registers.name_register,users.names_user,users.last_names_user, ' +
            ...
            'sells.currency_code,sells.id_user order by users.names_user', [initialDate, endDate]);
        res.json(response.rows);
    }
    ...
    async getProductosVentaById(req, res) {
        const id_sell = parseInt(req.params.id_sell);
        const response = await conection_1.default.query('SELECT * FROM products JOIN sell_products ON ' +
    ...
    async getProductoVentaById(req, res) {
        ...
        const response = await conection_1.default.query('SELECT * FROM sell_products where id_sell=$1 and id_product=$2', [id_sell, id_product]);
        res.json(response.rows);
    }
    async getPagosVentaById(req, res) {
        const id_sell = parseInt(req.params.id_sell);
        const response = await conection_1.default.query('SELECT distinct on (pays.id_pay)* FROM pays JOIN users ON ' +
    ...
    async getPagosVentaByDate(req, res) {
        const initialDate = req.params.initialDate;
        const endDate = req.params.endDate;
        const response = await conection_1.default.query("SELECT currency,c.currency_code,c.language_code,c.country_code, " +
            ...
            "c.currency_code=s.currency_code group by currency,c.currency_code,c.language_code,c.country_code ", [initialDate, endDate]);
        if (response.rowCount > 0) {
            res.json(response.rows);
        }
        else {
    ...
    async createPay(req, res) {
        ...
        try {
            await client.query('INSERT INTO pays (id_sell,type,reference,' +
                'mount,currency_code,exchange,id_user) values($1,$2,$3,$4,$5,$6,$7)', [sell.id_sell, pay.type, pay.reference, pay.mount, pay.currency_code,
                pay.exchange, pay.id_user]);
            await client.query('UPDATE sells SET total_sell=$1, total_paid=$2,state=$3 WHERE id_sell=$4', [sell.total_sell, sell.total_paid, sell.state, sell.id_sell]);
    ...
    async createVenta(req, res) {
        const { total_sell, currency_code, exchange, state, discount, type_sell, total_paid, id_user, id_client, sell_products, pays } = req.body;
        ...
        try {
            let response = await client.query('INSERT INTO sells (total_sell,currency_code,exchange,state,' +
                'discount,type_sell,total_paid,id_user,id_client) values($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id_sell', [total_sell, currency_code, exchange, state, discount, type_sell, total_paid, id_user, id_client]);
            ...
            pays.forEach(async (data) => {
                await client.query('INSERT INTO pays (id_sell,type,reference,' +
                    'mount,currency_code,exchange,id_user) values($1,$2,$3,$4,$5,$6,$7)', [response.rows[0].id_sell, data.type, data.reference, data.mount, data.currency_code,
    ...
    async actualizarVenta(req, res) {
        ...
        await conection_1.default.query('UPDATE sells SET total_sell=$1 WHERE id_sell=$1', [total_buy, id_provider, id_user, id_sell]);
        res.status(200).json({
    ...
    async actualizarProductosVenta(req, res) {
        ...
        await conection_1.default.query('UPDATE buy_products set id_sell=$1,id_product=$2,buy_price=$3,sell_price=$4,' +
    ...
    async eliminarVenta(req, res) {
        const id_sell = parseInt(req.params.id_sell);
        await conection_1.default.query('DELETE FROM sells WHERE id_sell=$1', [id_sell]);
        res.status(200).json({
