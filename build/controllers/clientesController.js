...
class ClientesController {
    async getClientes(req, res) {
        const response = await conection_1.default.query('SELECT * FROM clients WHERE id_client!=0 ORDER BY names_client ASC');
        res.json(response.rows);
    }
    async getAllClientes(req, res) {
        const response = await conection_1.default.query('SELECT * FROM clients ORDER BY names_client ASC');
        res.json(response.rows);
    }
    async getClienteById(req, res) {
        ...
        const response = await conection_1.default.query('SELECT * FROM clients WHERE id_client=$1 ORDER BY names_client ASC', [id_client]);
    ...
    async createCliente(req, res) {
        ...
        let response = await conection_1.default.query('INSERT INTO clients ' +
    ...
    async actualizarCliente(req, res) {
        const id_client = parseInt(req.params.id_client);
        const { document_type_client, document_number_client, names_client, last_names_client, state_client, city_client, street_client, phone_number_client, email_client } = req.body;
        await conection_1.default.query('UPDATE clients SET document_type_client=$1,document_number_client=$2,' +
    ...
    async eliminarCliente(req, res) {
        const id_client = parseInt(req.params.id_client);
        await conection_1.default.query('DELETE FROM clients WHERE id_client=$1', [id_client]);
        res.status(200).json({
