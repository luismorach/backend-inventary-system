...
class ProductosController {
    async getProductos(req, res) {
        const response = await conection_1.default.query('SELECT * FROM products p join taxes t ' +
            'on p.tax_id = t.tax_id ORDER BY name ASC');
        res.json(response.rows);
    }
    async getProductosMasVendidos(req, res) {
        const response = await conection_1.default.query('SELECT * FROM products ORDER BY sell_quantity DESC');
        res.json(response.rows);
    }
    async getProductosPorVencimiento(req, res) {
        const response = await conection_1.default.query('SELECT * FROM products ORDER BY expir DESC');
        res.json(response.rows);
    }
    async getProductoById(req, res) {
        ...
        const response = await conection_1.default.query('SELECT * FROM products WHERE id_product=$1 ORDER BY name ASC', [id_product]);
    ...
    async getProductoByBarcode(req, res) {
        ...
        const response = await conection_1.default.query('SELECT * FROM products WHERE barcode=$1 ORDER BY name ASC', [barcode]);
        if (response.rowCount > 0) {
            res.json(response.rows);
        }
        else {
    ...
    async getProductoByCategory(req, res) {
        ...
        const response = await conection_1.default.query('SELECT * FROM products WHERE id_category=$1 ORDER BY name ASC', [id_category]);
        res.json(response.rows);
    }
    async createProducto(req, res) {
        const { barcode, name, garanty, mark, model, can_expir, expir, time_garanty, id_category, discount, tax_id, time_unit } = req.body;
        await conection_1.default.query('INSERT INTO products ' +
    ...
    async actualizarProducto(req, res) {
        ...
        await conection_1.default.query('UPDATE products SET barcode=$1,name=$2,garanty=$3,mark=$4,' +
    ...
    async eliminarProducto(req, res) {
        const id_product = parseInt(req.params.id_product);
        await conection_1.default.query('DELETE FROM products WHERE id_product=$1', [id_product]);
        res.status(200).json({
