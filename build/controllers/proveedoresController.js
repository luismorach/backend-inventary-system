...
class ProveedoresController {
    async getProveedores(req, res) {
        const response = await conection_1.default.query('SELECT * FROM providers ORDER BY name_provider ASC');
    ...
    async getProveedoresById(req, res) {
        ...
        const response = await conection_1.default.query('SELECT * FROM providers WHERE id_provider=$1 ORDER BY name_provider ASC', [id_provider]);
    ...
    async createProveedor(req, res) {
        ...
        await conection_1.default.query('INSERT INTO providers ' +
    ...
    async actualizarProveedor(req, res) {
        const id_provider = parseInt(req.params.id_provider);
        const { document_type, document_number, name_provider, address_provider, name_boss, phone_number, email, } = req.body;
        await conection_1.default.query('UPDATE providers SET document_type=$1,document_number=$2,' +
    ...
    async eliminarProveedor(req, res) {
        const id_provider = parseInt(req.params.id_provider);
        await conection_1.default.query('DELETE FROM providers WHERE id_provider=$1', [id_provider]);
        res.status(200).json({
