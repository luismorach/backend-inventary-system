...
class ComprasController {
    async getCompras(req, res) {
        const response = await conection_1.default.query('SELECT * FROM buys ORDER BY date,time DESC ');
    ...
    async getCompraById(req, res) {
        ...
        const response = await conection_1.default.query("SELECT distinct on (b.id_buy) * " +
    ...
    async getProductosCompraById(req, res) {
        const id_buy = parseInt(req.params.id_buy);
        const response = await conection_1.default.query('SELECT * FROM buy_products WHERE id_buy=$1 order by id_buy', [id_buy]);
        if (response.rowCount > 0) {
            res.json(response.rows);
        }
    }
    async getProductoCompraById(req, res) {
        ...
        const response = await conection_1.default.query('SELECT * FROM buy_products WHERE id_buy=$1 and id_product=$2', [id_buy, id_product]);
        res.json(response.rows);
    }
    ...
    async actualizarCompra(req, res) {
        ...
        await conection_1.default.query('UPDATE buys SET total_buy=$1,id_provider=$2,id_user=$3 WHERE id_buy=$4', [total_buy, id_provider, id_user, id_buy]);
        res.status(200).json({
    ...
    async actualizarProductosCompra(req, res) {
        ...
        await conection_1.default.query('UPDATE buy_products set id_buy=$1,id_product=$2,buy_price=$3,sell_price=$4,' +
    ...
    async eliminarCompra(req, res) {
        const id_buy = parseInt(req.params.id_buy);
        await conection_1.default.query('DELETE FROM buys WHERE id_buy=$1', [id_buy]);
        res.status(200).json({
