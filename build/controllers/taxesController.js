...
class TaxesController {
    async getTaxes(req, res) {
        const response = await conection_1.default.query('SELECT * FROM taxes ORDER BY tax_name ASC');
        res.json(response.rows);
    }
    async getTaxById(req, res) {
        const tax_id = parseInt(req.params.tax_id);
        const response = await conection_1.default.query('SELECT * FROM taxes WHERE tax_id=$1 ORDER BY tax_name ASC', [tax_id]);
    ...
    async createTax(req, res) {
        ...
        await conection_1.default.query('INSERT INTO taxes (tax_name,tax_rate,show_tax) values($1,$2,$3)', [tax_name, tax_rate, show_tax]);
        res.status(200).json({
    ...
    async updateTax(req, res) {
        const tax_id = parseInt(req.params.tax_id);
        const { tax_name, tax_rate, show_tax } = req.body;
        await conection_1.default.query('UPDATE taxes SET tax_name=$1,tax_rate=$2,show_tax=$3 WHERE tax_id=$4', [tax_name, tax_rate, show_tax, tax_id]);
        res.status(200).json({
    ...
    async deleteTax(req, res) {
        const tax_id = parseInt(req.params.tax_id);
        await conection_1.default.query('DELETE FROM taxes WHERE tax_id=$1', [tax_id]);
        res.status(200).json({
